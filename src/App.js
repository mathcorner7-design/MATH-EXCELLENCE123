...
window.openAdminClass = openAdminClass;
window.setOpenAdminClass = setOpenAdminClass;
        ...
        {classes.map(cls => {
          ...
          return (
            <div key={cls} className="space-y-3">
              {/* 📁 মেইন ক্লাস হেডার */}
              <div onClick={() => window.setOpenAdminClass(window.openAdminClass === cls ? null : cls)} className="flex justify-between items-center cursor-pointer p-3 bg-white/5 rounded-xl border border-white/5 hover:bg-white/10 transition-all" >
                <h4 className="text-[10px] font-black text-blue-400 uppercase italic flex items-center gap-2">
                  📁 Class {cls}
                </h4>
                <span className="text-[9px] font-black bg-slate-800 text-slate-400 px-2 py-0.5 rounded">
                  {classItems.length} Exams
                </span>
              </div>
                ...
                <div className="pl-4 space-y-3 mt-2 animate-in slide-in-from-top-2 duration-200">
                  ...
                  {title === "Practice Sets" ? (
                    chapters.map(chName => {
                      ...
                      // 💡 মেইন ফিক্স: আইডি তৈরি করা হচ্ছে যাতে ব্রাউজার মনে রাখতে পারে কোনটা ক্লিক করা হয়েছিল
                      const detailsId = `admin-ch-${cls}-${chName.replace(/\s+/g, '-')}`;
                      ...
                      return (
                        <details 
                          ...
                          id={detailsId}
                          onClick={(e) => {
                            // স্টুডেন্ট প্যানেলের মতো ফায়ারবেস আপডেটের পরও যাতে খোলা থাকে তার মেমোরি ট্র্যাকিং
                            const isOpen = e.currentTarget.hasAttribute('open');
                            localStorage.setItem(detailsId, !isOpen ? 'open' : 'closed');
                          }}
                          // কোড যখনই নতুন করে লোড হবে, সে চেক করবে আগে এটা খোলা ছিল কিনা
                          ref={(el) => {
                            if (el) {
                              const savedState = localStorage.getItem(detailsId);
                              if (savedState === 'open') el.setAttribute('open', 'true');
                            }
                          }}
                          className="group bg-black/40 border border-slate-800/60 rounded-2xl p-3 [&_summary::-webkit-details-marker]:hidden"
                        >
  ...
  <div id={`admin-pdf-drop-${item.id}`} className="hidden absolute right-0 top-9 z-[3000] w-44 p-2 bg-slate-950 border border-white/10 rounded-2xl shadow-2xl flex flex-col gap-2 animate-in fade-in zoom-in duration-150">
    ...
    {item.answerPdfUrl ? (
      ...
      <div className="flex items-center gap-2 p-2.5 text-slate-600 text-[9px] font-bold uppercase italic"><Lock size={10} /> No Ans</div>
                    ...
                    /* 🔴 লাইভ মক এক্সামের জন্য সাধারণ লিস্ট ডিজাইন */
                    classItems.map((item) => (
  ...
  <div id={`admin-pdf-drop-${item.id}`} className="hidden absolute right-0 top-9 z-[3000] w-44 p-2 bg-slate-950 border border-white/10 rounded-2xl shadow-2xl flex flex-col gap-2 animate-in fade-in zoom-in duration-150">
    ...
    {item.answerPdfUrl ? (
      ...
      <div className="flex items-center gap-2 p-2.5 text-slate-600 text-[9px] font-bold uppercase italic"><Lock size={10} /> No Ans</div>
                  ...
                  {/* ✏️ রাজকীয় ১১টি অপশন যুক্ত ফুল এডিট মোডাল উইন্ডো */}
  ...
  {r.status === "BANNED" && (
    ...
    <div className="relative inline-block print:hidden">
        <button 
            ...
            title="View QP / Answers"
        >
            <Settings2 size={18} />
        </button>

        {/* ড্রপডাউন সাব-মেনু (যা এখন বাটনের ঠিক বাম পাশে বা নিচে পজিশন লক হয়ে খুলবে) */}
        <div className="hidden absolute right-0 top-11 z-[2500] w-44 p-2 bg-slate-950 border border-white/10 rounded-2xl shadow-2xl flex flex-col gap-2 animate-in fade-in zoom-in duration-150">
            ...
            {r.answerPdfUrl ? (
                <a href={r.answerPdfUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-2.5 rounded-xl bg-white/5 hover:bg-green-950/40 border border-transparent hover:border-green-900/50 transition-all text-left text-white">
...
status: (isBanned || forcedBan) ? "BANNED" : "COMPLETED", obtained: totalObtainedMarks, total: totalPossibleMarks, date: d.toLocaleDateString('en-GB'), timestamp: Date.now(), details: detailResults, answerPdfUrl: exam.answerPdfUrl || "", fileUrl: exam.fileUrl || "", timeTaken: timeDuration, bonus: 0 });
      }
                  ...
                  return (
                    <div key={r.id} className="w-full bg-slate-900/60 rounded-[2rem] border border-white/10 shadow-sm flex items-center p-4 md:p-5 gap-3 md:gap-6 hover:shadow-md transition-all group print-card">
                      <div className="flex-1 min-w-0 border-l-4 md:border-l-8 border-blue-600 pl-3 md:pl-5">
                        ...
                        {hasPending && <p className="text-[7px] md:text-[8px] font-black text-orange-400 uppercase italic mt-0.5 animate-pulse">WAITING FOR SIR'S EVALUATION</p>}
