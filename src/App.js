...
const App = () => {
  ...
  return (
    <div className="min-h-screen font-sans text-white select-none flex flex-col items-center overflow-x-hidden transition-all duration-700 bg-black" style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.8)), url(${APP_BACKGROUND_URL})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }} >
      <style>{`
        @media print { body { background: white !important; overflow: visible !important; } header, nav, .print-hide, button, .lucide { display: none !important; } .min-h-screen { min-height: auto !important; background: none !important; } main { padding: 0 !important; width: 100% !important; color: black !important; } .print-full-report { display: block !important; position: static !important; width: 100% !important; height: auto !important; } .print-card { border: 2px solid #ddd !important; margin-bottom: 15px !important; color: black !important; background: white !important; } .text-white { color: black !important; } }
      `}</style>
      ...
      <header className="bg-black/60 backdrop-blur-xl border-b border-white/10 sticky top-0 z-50 shadow-2xl px-6 py-2 flex justify-between items-center w-full max-w-6xl print:hidden">
        ...
        <div className="text-right">
          <p className="text-[9px] font-bold text-slate-500 italic uppercase">Anshu Sir</p>
      ...
      <main className="w-full max-w-5xl p-4 mb-20 flex flex-col items-center">
            ...
            <div className="bg-black/60 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl border-2 border-white/10">
              ...
              <div className="mt-10 overflow-hidden rounded-2xl border border-white/10 shadow-2xl">
                <table className="w-full text-left text-[11px] md:text-xs">
                  <thead><tr className="bg-blue-700/50 text-white uppercase italic font-black"><th className="p-4">Features</th><th className="p-4 text-center">Guest</th><th className="p-4 text-center text-yellow-400">Registered</th></tr></thead>
                  <tbody className="bg-black/40 font-bold text-slate-300">
                    ...
                    <tr className="border-b border-white/5"><td className="p-4">Test Format</td><td className="p-4 text-center">Only MCQ</td><td className="p-4 text-center text-green-400">MCQ + Written</td></tr>
                    <tr className="border-b border-white/5"><td className="p-4">Reports</td><td className="p-4 text-center">Marks</td><td className="p-4 text-center text-green-400">Full Review</td></tr>
              ...
              <div className="space-y-3">
                {activityLogs.slice(0, 10).map(log => (
                  <div key={log.id} className="p-2.5 bg-white/5 rounded-xl flex justify-between items-center border-l-4 border-blue-600 shadow-sm transition-all hover:bg-white/10">
                    ...
                    <div className="text-right text-[7px] font-bold text-slate-500 uppercase leading-tight">RECENT</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'live' && (
          <div className="space-y-4 w-full text-left print:hidden">
            <h2 className="font-bold uppercase text-blue-300 border-b border-white/10 pb-2 text-[10px] flex items-center gap-2 bg-black/40 p-2 rounded-lg backdrop-blur-md italic"><Clock size={14} className="text-red-500" /> Ongoing Live Mocks</h2>
            {ongoingLive.map((m, i) => (
              <div key={m.id} className="bg-black/60 backdrop-blur-xl p-4 rounded-2xl shadow-xl flex justify-between items-center border border-white/10">
                <div className="flex-1 pr-4"><div className="flex items-center flex-wrap"><h3 className="text-sm font-black uppercase italic text-white">{i + 1}. {m.name}</h3><LevelBadge level={m.level} /></div><LiveCountdown timestamp={m.timestamp} /></div>
            ...
            {(() => {
              ...
              return classes.map(cls => (
                <div key={cls} className="space-y-4">
                  <h2 className="font-black uppercase text-blue-400 border-b-2 border-blue-900/50 pb-2 text-xs flex items-center gap-2 italic pl-2"><BookOpen size={16} /> Class {cls}</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {allMocks.filter(m => (m.class || 'Other') === cls).map((p, i) => (
                      <div key={p.id} className="bg-black/60 backdrop-blur-xl p-4 rounded-2xl shadow flex justify-between items-center border border-white/10 hover:border-blue-500/50 transition-all">
                        <div className="flex-1 pr-4"><div className="flex items-center flex-wrap"><h3 className="font-bold uppercase text-xs italic text-white">{i + 1}. {p.name}</h3><LevelBadge level={p.level} /></div><p className="text-[9px] font-bold text-slate-500 uppercase mt-1">Time: {p.hours || 0}h {p.minutes || 0}m</p></div>
          ...
          <div className="w-full flex flex-col items-center">
            {!currentUser ? (
              <div className="max-w-md w-full mx-auto mt-20 p-10 bg-slate-950 backdrop-blur-xl rounded-3xl shadow-2xl text-center border-t-8 border-blue-700 border-x border-b border-white/10 print:hidden">
                ...
                <button onClick={handleGoogleLogin} className="w-full bg-white text-black py-4 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-3 hover:bg-blue-400 transition-all shadow-xl active:scale-95"><LogIn size={20}/> Authenticate with Google</button>
              </div>
            ) : (
...
const TeacherZoneMainView = ({ liveMocks, practiceSets, students, studentResults }) => {
  ...
  const handleQuickAdd = async () => {
    ...
    setQaName(''); setQaLink(''); setQaKey(''); setQaMarks(''); setQaNeg('0'); alert(`Added!`);
  };
              ...
              {items.filter(m => (m.class || 'Other') === cls).map((item) => (
                <div key={item.id} className="bg-slate-900/60 rounded-2xl border border-white/10 overflow-hidden transition-all">
                  <div onClick={() => setExpandedId(expandedId === item.id ? null : item.id)} className="p-4 flex justify-between items-center cursor-pointer hover:bg-white/5 group">
                    <div className="flex-1"><div className="flex items-center gap-3"><div className={`w-2 h-2 rounded-full ${item.isPublished ? 'bg-green-500 animate-pulse' : 'bg-slate-700'}`}></div><span className="text-xs font-black uppercase text-white">{item.name}</span>{item.level && <span className="text-[7px] bg-blue-900 px-1.5 py-0.5 rounded text-blue-300 font-black italic">{item.level}</span>}</div></div>
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={async (e) => { e.stopPropagation(); if(window.confirm("Delete?")) await deleteDoc(doc(db, type === 'live' ? 'liveMocks' : 'practiceSets', item.id)); }} className="p-2 text-slate-600 hover:text-red-500"><Trash2 size={16}/></button>
                      <ChevronRight size={18} className={`${expandedId === item.id ? 'rotate-90' : ''}`} />
                    </div>
                  </div>
                  {expandedId === item.id && (
                    <div className="p-5 border-t border-white/5 bg-black/40 space-y-4">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-end">
                        <div className="flex items-center gap-2"><input type="checkbox" checked={item.isGuestEnabled} onChange={(e) => updateField(item.id, type, 'isGuestEnabled', e.target.checked)} className="accent-green-500 w-4 h-4" /><p className="text-[10px] font-black text-green-400">Guest</p></div>
                        <div><p className="text-[8px] font-black text-blue-400">Class</p><select value={item.class || '10'} onChange={(e) => updateField(item.id, type, 'class', e.target.value)} className="w-full p-2 bg-black border border-white/10 rounded-xl text-white text-xs font-black">{[5,6,7,8,9,10,11,12].map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                        <div className="md:col-span-2"><p className="text-[8px] font-black text-yellow-500">Level</p><select value={item.level || 'Moderate'} onChange={(e) => updateField(item.id, type, 'level', e.target.value)} className="w-full p-2 bg-black border border-white/10 rounded-xl text-white text-xs font-black">{['Easy', 'Moderate', 'Hard'].map(lvl => <option key={lvl} value={lvl}>{lvl}</option>)}</select></div>
                      </div>
                    </div>
                  )}
                </div>
    ...
    <div className="w-full flex flex-col items-center">
      <div className="bg-slate-950/80 backdrop-blur-xl p-6 rounded-[2.5rem] shadow-2xl border-t-8 border-blue-700 w-full mb-8 text-left border-x border-b border-white/5">
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-black text-[10px] uppercase flex items-center gap-2 italic text-blue-400"><Zap size={20} /> Quick Add</h3>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="flex items-center gap-2"><input type="checkbox" checked={qaGuest} onChange={(e) => setQaGuest(e.target.checked)} className="accent-blue-500" /><p className="text-[9px] font-black text-slate-400 italic">Guest Access</p></div>
            <div><p className="text-[8px] font-black text-blue-400">Class</p><select value={qaClass} onChange={(e) => setQaClass(e.target.value)} className="w-full p-2 bg-black border border-white/10 rounded-xl text-white text-[10px] font-black outline-none">{[5, 6, 7, 8, 9, 10, 11, 12].map(c => <option key={c} value={c}>{c}</option>)}</select></div>
            <div><p className="text-[8px] font-black text-yellow-500">Level</p><select value={qaLevel} onChange={(e) => setQaLevel(e.target.value)} className="w-full p-2 bg-black border border-white/10 rounded-xl text-white text-[10px] font-black outline-none">{['Easy', 'Moderate', 'Hard'].map(lvl => <option key={lvl} value={lvl}>{lvl}</option>)}</select></div>
          </div>
          <button onClick={handleQuickAdd} className="w-full bg-blue-700 text-white py-4 rounded-[1.5rem] font-black text-[11px] uppercase shadow-2xl flex items-center justify-center gap-3 border-b-4 border-blue-900 italic tracking-tighter"><Send size={18} /> Deploy to Registry</button>
        </div>
      </div>
      ...
      <div className="bg-black/60 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl border-t-8 border-slate-900 w-full mb-20 text-center border-x border-b border-white/5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {students.map((std) => (
            <div key={std.id} className="relative p-5 bg-white/5 border border-white/10 rounded-[2rem] flex flex-col items-center">
              <input type="text" defaultValue={std.name} onBlur={async (e) => { if (e.target.value !== std.name) await setDoc(doc(db, "students", std.id), { name: e.target.value.toUpperCase() }, { merge: true }); }} className="bg-transparent text-center font-black uppercase text-white outline-none italic tracking-tighter" />
              <div className="mt-2 flex items-center gap-2 bg-blue-950 px-3 py-1 rounded-full"><Lock size={10} className="text-blue-400" /><input type="text" defaultValue={std.studentCode} onBlur={async (e) => { if (e.target.value !== std.studentCode) await setDoc(doc(db, "students", std.id), { studentCode: e.target.value }, { merge: true }); }} className="bg-transparent text-[10px] font-black text-blue-400 w-20 text-center outline-none" /></div>
              <div className="flex gap-3 mt-6">
                <button onClick={() => setSelectedStudent(std)} className="px-5 py-1.5 bg-slate-800 rounded-full text-[10px] font-black uppercase italic hover:bg-blue-600 transition-all">Reports</button>
                <button onClick={async (e) => { if (window.confirm(`Delete?`)) await deleteDoc(doc(db, "students", std.id)); }} className="p-2 bg-red-950/40 text-red-500 rounded-full"><Trash2 size={16} /></button>
              </div>
            </div>
          ))}
          <button onClick={async () => { const n = prompt("Name:"); const c = prompt("Code:"); if (n) await addDoc(collection(db, "students"), { name: n.toUpperCase(), studentCode: c || "" }); }} className="p-8 border-4 border-dashed border-white/10 rounded-[2.5rem] text-[12px] font-black text-slate-600 uppercase hover:text-blue-500 transition-all">+ REGISTER</button>
        </div>
      </div>
...
const GrowthSectionView = ({ results, students }) => {
  ...
  const handleVerify = () => {
    ...
    if (student && student.studentCode?.toString().trim() === vCode.trim()) { setIsVerified(true); } else { alert("INVALID CODE"); }
  };
  return (
      ...
      {!sel ? (
        ...
        <div className="space-y-6 animate-in slide-in-from-right-20 duration-700 print-full-report">
          ...
          <div className="bg-black/90 rounded-[3rem] shadow-2xl overflow-hidden border-2 border-white/10 flex flex-col print-full-report">
            <div className="bg-blue-700 p-8 text-white text-center relative overflow-hidden"><h2 className="text-2xl font-black uppercase italic tracking-tighter mb-2">Performance Transcript</h2><div className="inline-block bg-white/20 px-6 py-1.5 rounded-full border border-white/30"><p className="text-sm font-black uppercase italic">{sel}</p></div></div>
            <div className="p-4 md:p-6 space-y-4 bg-white/5 print:bg-white h-auto">
              {(() => {
                const stdResults = results.filter(r => r.name === sel).sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
                return stdResults.map((r) => (
                  <div key={r.id} className="w-full bg-slate-900/60 rounded-[2rem] border border-white/10 shadow-sm flex items-center p-4 md:p-5 gap-3 md:gap-6 hover:shadow-md transition-all group print-card">
                    <div className="flex-1 min-w-0 border-l-4 border-blue-600 pl-3 md:pl-5"><p className="text-[8px] md:text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-0.5 italic">Exam Unit Log</p><p className="text-xs md:text-lg font-black uppercase italic text-white leading-tight whitespace-normal break-words tracking-tighter">{r.exam}</p><p className="text-[8px] md:text-[9px] font-black text-blue-400 uppercase italic mt-1 tracking-widest">{new Date(r.timestamp).toLocaleDateString('en-GB')}</p></div>
...
const InteractiveExamHall = ({ exam, onFinish, studentsList }) => {
  ...
  const submitExam = async () => {
    try {
      ...
      if (!exam.isGuest) { await addDoc(collection(db, "results"), { name: finalStudentName, exam: exam.name, percent, obtained: totalObtainedMarks, total: totalPossibleMarks, date: d.toLocaleDateString('en-GB'), timestamp: Date.now(), details: detailResults }); }
